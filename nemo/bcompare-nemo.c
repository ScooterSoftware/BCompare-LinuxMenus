...
void nemo_module_list_types(const GType **types, gint *n_types);
...
{
	g_signal_emit_by_name((NemoMenuProvider *)bcobj, "items_updated");
}
...
static gchar * nemo_to_path(NemoFileInfo* file)
{
	return g_filename_from_uri(nemo_file_info_get_uri(file), NULL, NULL);
}
		...
		gboolean IsDir)
{
	...
	item = nemo_menu_item_new("BCompareExt::select_left",
							   MenuStr->str,
							   HintStr->str,
							   "bcomparehalf32");
...
static BcMenuItem * select_center_mitem(BCompareExt *bcobj)
{
	...
	item = nemo_menu_item_new("BCompareExt::select_center",
								  "Select Center File",
...
static BcMenuItem * edit_file_mitem(BCompareExt *bcobj)
{
	...
	item = nemo_menu_item_new("BCompareExt::edit_file",
								  MenuStr->str,
		...
		int SelectedCnt)
{
	...
	item = nemo_menu_item_new(NameStr->str,
								  MenuStr->str,
								  HintStr->str,
								  "bcomparefull32" /* icon name */);
	g_signal_connect(item, "activate",
		...
		int SelectedCnt)
{
	...
	item = nemo_menu_item_new("BCompareExt::sync",
								  MenuStr->str,
								  HintStr->str,
								  "bcomparefull32" /* icon name */);
	g_signal_connect(item, "activate",
		...
		int SelectedCnt)
{
	...
	item = nemo_menu_item_new("BCompareExt::merge",
								  MenuStr->str,
								  HintStr->str,
								  "bcomparefull32" /* icon name */);
	g_signal_connect(item, "activate",
	...
	if ((!bcobj->LeftIsDir) &&
		...
		if (SelectedCnt < 3) {
			...
			if (bcobj->CompareUsingMenuType == CurrentMenuType &&
				...
				item = nemo_menu_item_new("BeyondCompareExt::compareusing",
										"Compare using",
					...
					GList *files)
{
	...
	FirstIsDir =
		file_is_dir(bcobj, nemo_to_path((NemoFileInfo *)files->data));

	SelectedCnt = g_list_length(files);
	if (SelectedCnt > 1) {
		for (Cnt = 1; Cnt < g_list_length(files); Cnt++) {
			if (FirstIsDir != file_is_dir(bcobj,
					   nemo_to_path((NemoFileInfo *)(g_list_nth_data(files, Cnt))))) {
				return NULL;
			}
		}
	}
	...
	if (SelectedCnt == 3) {
		...
		bcobj->CenterFile = g_string_new(nemo_to_path(
			(NemoFileInfo *)g_list_nth_data(files, 2)));
	}
	if (SelectedCnt >= 2) {
		if (bcobj->LeftFile != NULL)
			g_string_free(bcobj->LeftFile, TRUE);
		bcobj->LeftFile = g_string_new(nemo_to_path(
			(NemoFileInfo *)g_list_nth_data(files, 0)));

		bcobj->LeftIsDir = FirstIsDir;

		if (bcobj->RightFile != NULL)
			g_string_free(bcobj->RightFile, TRUE);
		bcobj->RightFile = g_string_new(nemo_to_path(
			(NemoFileInfo *)g_list_nth_data(files, 1)));
	}
	if (SelectedCnt == 1) {
		if (leftfileptr != NULL) {
			if (bcobj->LeftFile != NULL)
	...
	if (ret != NULL) {
		item = nemo_menu_item_new("BeyondCompareExt::Top",
								  "Beyond Compare",
...
/*************************************************************
 *
 * Nemo Extension Interface functions
 *
 *************************************************************/
...
void
nemo_module_list_types(const GType **types, gint *n_types)
{
	*types = type_list;
