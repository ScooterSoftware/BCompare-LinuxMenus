...
void caja_module_list_types(const GType **types, gint *n_types);
...
{
	g_signal_emit_by_name((CajaMenuProvider *)bcobj, "items_updated");
}
...
static gchar * caja_to_path(CajaFileInfo* file)
{
	return g_filename_from_uri(caja_file_info_get_uri(file), NULL, NULL);
}
					...
					GList *files)
{
	...
	FirstIsDir =
		file_is_dir(bcobj, caja_to_path((CajaFileInfo *)files->data));

	SelectedCnt = g_list_length(files);
	if (SelectedCnt > 1) {
		for (Cnt = 1; Cnt < g_list_length(files); Cnt++) {
			if (FirstIsDir != file_is_dir(bcobj,
					   caja_to_path((CajaFileInfo *)(g_list_nth_data(files, Cnt))))) {
				return NULL;
			}
		}
	}
	...
	if (SelectedCnt == 3) {
		...
		bcobj->CenterFile = g_string_new(caja_to_path(
			(CajaFileInfo *)g_list_nth_data(files, 2)));
	}
	if (SelectedCnt >= 2) {
		if (bcobj->LeftFile != NULL)
			g_string_free(bcobj->LeftFile, TRUE);
		bcobj->LeftFile = g_string_new(caja_to_path(
			(CajaFileInfo *)g_list_nth_data(files, 0)));

		bcobj->LeftIsDir = FirstIsDir;

		if (bcobj->RightFile != NULL)
			g_string_free(bcobj->RightFile, TRUE);
		bcobj->RightFile = g_string_new(caja_to_path(
			(CajaFileInfo *)g_list_nth_data(files, 1)));
	}
	if (SelectedCnt == 1) {
		if (leftfileptr != NULL) {
			if (bcobj->LeftFile != NULL)
...
/*************************************************************
 *
 * Caja Extension Interface functions
 *
 *************************************************************/
...
void
caja_module_list_types(const GType **types, gint *n_types)
{
	*types = type_list;
