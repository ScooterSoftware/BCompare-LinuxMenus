...
{
	/*g_signal_emit_by_name(bcobj, "items_updated"); */
}
...
static gchar * thunarx_to_path(ThunarxFileInfo* file)
{
	return g_filename_from_uri(thunarx_file_info_get_uri(file), NULL, NULL);
}
...
static GList * beyondcompare_get_file_actions(
					...
					GList *files)
{
	...
	FirstIsDir =
		file_is_dir(bcobj, thunarx_to_path((ThunarxFileInfo *)files->data));

	SelectedCnt = g_list_length(files);
	if (SelectedCnt > 1) {
		for (Cnt = 1; Cnt < g_list_length(files); Cnt++) {
			if (FirstIsDir != file_is_dir(bcobj,
						thunarx_to_path((ThunarxFileInfo *)(g_list_nth_data(files, Cnt))))) {
				return NULL;
			}
		}
	}
	...
	if (SelectedCnt == 3) {
		...
		bcobj->CenterFile = g_string_new(thunarx_to_path(
			(ThunarxFileInfo *)g_list_nth_data(files, 2)));
	}
	if (SelectedCnt >= 2) {
		if (bcobj->LeftFile != NULL)
			g_string_free(bcobj->LeftFile, TRUE);
		bcobj->LeftFile = g_string_new(thunarx_to_path(
			(ThunarxFileInfo *)g_list_nth_data(files, 0)));

		bcobj->LeftIsDir = FirstIsDir;

		if (bcobj->RightFile != NULL)
			g_string_free(bcobj->RightFile, TRUE);
		bcobj->RightFile = g_string_new(thunarx_to_path(
			(ThunarxFileInfo *)g_list_nth_data(files, 1)));
	}
	if (SelectedCnt == 1) {
		if (leftfileptr != NULL) {
			if (bcobj->LeftFile != NULL)
...
THUNARX_DEFINE_TYPE_WITH_CODE(BCompareExt, bcompare_ext, G_TYPE_OBJECT,
		THUNARX_IMPLEMENT_INTERFACE(THUNARX_TYPE_MENU_PROVIDER,
						beyondcompare_menu_provider_init));

/*************************************************************
 *
 * Thunar Extension Interface functions
 *
 *************************************************************/
...
thunar_extension_initialize(ThunarxProviderPlugin* plugin)
{
  ...
  if(G_LIKELY(mismatch == NULL)) {
	g_message("Initializing bcompare-ext extension");

	/* register the types provided by this plugin */
	bcompare_ext_register_type(plugin);
